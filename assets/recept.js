...
(function () {
  ...
  /* 🐛 FIX "kockläge funkar inte": recept skapade via nytt-recept.html
     kan ha TOM <ol> – stegen bor då i .machine-step-rutorna. Bygg stegen
     av maskinstegen istället så kockläget alltid finns. */
  if (!steps.length) {
    document.querySelectorAll('.machine-step').forEach(function (ms) {
      var h = (ms.querySelector('h3') || {}).textContent || '';
      var why = (ms.querySelector('.why') || {}).textContent || '';
      if (!h.trim() && !why.trim()) return;
      var card = ms.closest('.card');
      var heading = card ? ((card.querySelector('h2') || {}).textContent || '') : '';
      steps.push({
        text: '<b>' + h.trim() + '</b>' + (why ? '<div style="font-size:.7em;opacity:.85;margin-top:12px;">' + why.trim() + '</div>' : ''),
        plain: h.trim() + '. ' + why.trim(),
        section: heading.trim() || '⚙️ Maskinsteg', done: false
      });
    });
  }
  ...
  function open() {
    ...
    // Fortsätt där du var? (egen fråga i overlayn – confirm() blockeras av Brave!)
    idx = 0;
    var fragaFortsatt = (saved.idx > 0 && saved.idx < steps.length);
    overlay = document.createElement('div');
    overlay.className = 'no-print';
    /* 📱 padding skalar med skärmen + safe-area (iPhone-hak/hemlinje) */
    overlay.style.cssText = 'position:fixed;inset:0;background:#2c3e50;color:#fff;z-index:1000;display:flex;flex-direction:column;' +
      'padding:max(clamp(10px,2.5vw,20px),env(safe-area-inset-top)) max(clamp(10px,2.5vw,20px),env(safe-area-inset-right)) ' +
      'max(clamp(10px,2.5vw,20px),env(safe-area-inset-bottom)) max(clamp(10px,2.5vw,20px),env(safe-area-inset-left));font-family:inherit;';
    ...
    render();
    /* Fortsätt-frågan som banner ÖVER stegvyn (ett tryck – aldrig confirm) */
    if (fragaFortsatt) {
      var fb = document.createElement('div');
      fb.id = 'cmResume';
      fb.style.cssText = 'position:absolute;left:16px;right:16px;bottom:110px;background:#e67e22;color:#fff;' +
        'border-radius:14px;padding:14px 16px;display:flex;gap:10px;align-items:center;flex-wrap:wrap;' +
        'box-shadow:0 10px 30px rgba(0,0,0,.4);z-index:6;font-size:.95rem;';
      fb.innerHTML = '<span style="flex:1;min-width:140px;"><b>👋 Välkommen tillbaka!</b> Du var på steg ' + (saved.idx + 1) + '.</span>' +
        '<button id="cmResYes" style="background:#fff;color:#e67e22;border:none;border-radius:10px;padding:10px 16px;font-weight:700;cursor:pointer;font-family:inherit;">Fortsätt där</button>' +
        '<button id="cmResNo" style="background:rgba(0,0,0,.25);color:#fff;border:none;border-radius:10px;padding:10px 14px;font-weight:700;cursor:pointer;font-family:inherit;">Från början</button>';
      overlay.appendChild(fb);
      fb.querySelector('#cmResYes').onclick = function () {
        idx = saved.idx; fb.remove(); render();
      };
      fb.querySelector('#cmResNo').onclick = function () { fb.remove(); };
    }
  }
  ...
  function render() {
    ...
    overlay.innerHTML =
      ...
      '</div>' +
      /* Navigering – 📱 knapphöjd/textstorlek skalar med skärmen */
      '<div style="display:flex;gap:clamp(8px,2vw,12px);">' +
        '<button id="cmPrev" style="flex:1;background:rgba(255,255,255,.15);color:#fff;border:none;border-radius:14px;padding:clamp(14px,3.5vw,20px);font-size:clamp(1.05rem,3vw,1.25rem);font-weight:700;cursor:pointer;min-height:56px;' + (idx === 0 ? 'opacity:.3;' : '') + '">←</button>' +
        '<button id="cmNext" style="flex:3;background:#27ae60;color:#fff;border:none;border-radius:14px;padding:clamp(14px,3.5vw,20px);font-size:clamp(1.05rem,3vw,1.25rem);font-weight:700;cursor:pointer;min-height:56px;">' +
          (idx === steps.length - 1 ? '✅ Klart!' : 'Nästa →') + '</button>' +
      '</div>' +
      '<div style="text-align:center;opacity:.5;font-size:clamp(.68rem,1.8vw,.8rem);margin-top:6px;">Svep eller piltangenter · skärmen hålls tänd · position sparas</div>' +
