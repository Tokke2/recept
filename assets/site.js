...
(function () {
  ...
  function checkMeta() {
    ...
    var recommended = ['recept:energi'];
    var missing = required.filter(function (n) { return !document.querySelector('meta[name="' + n + '"]'); });
    var missingRec = recommended.filter(function (n) { return !document.querySelector('meta[name="' + n + '"]'); });
    ...
    if (missingRec.length) {
      console.info('[Maskinkök] 💡 Tips: lägg till ' + missingRec.join(', ') +
        ' för automatisk energikostnadstabell. Format: "maskin-id: 50 min" eller "Namn: 550 W x 6 min"');
    }
